...
const Page = ({ challanno, dlnumber }) => {
  ...
  async function dlSuspensionRecommendedViewUser() {
    try {
      ...
      await fetch(
        `${serviceUrl}get-dl-suspension-recommendation-details?RTOCode=${user?.RTOCode}&ChallanNumber=${challanno}&DLStatus=0`,
        requestOptions
      )
  ...
  const recommededHandle = () => {
    setRecommededAlert(false);
    router.push(`/dl-suspensions/${pendingType}`);
  };
    ...
    <div className="p-6">
      ...
      {apiData && dlOnwer ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <CardContent className="flex flex-col md:flex-row gap-8">
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="font-medium">{dlOnwer?.data?.name ?? "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">
                    Son/Wife/Daughter of
                  </p>
                  <p className="font-medium">{dlOnwer?.data?.fatherName}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">DL Number</p>
                  <p className="font-medium">{dlOnwer?.data?.dlno ?? "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Date of Birth</p>
                  <p className="font-medium">
                    {new Date(dlOnwer?.data?.dob ?? "N/A").toLocaleDateString(
                      "en-GB",
                      {
                        day: "2-digit",
                        month: "2-digit",
                  ...
                  <div className="w-[250px] h-[250px] flex items-center justify-center rounded-lg cursor-pointer">
                    {base64ImageDoc && base64ImageDoc?.vehicleImpoundImg ? (
                      <Image
                        ...
                        className="rounded-lg object-cover"
                        onClick={() =>
                          setSelectedImage(base64ImageDoc?.vehicleImpoundImg)
                        }
                      />
                    ) : (
                      <div className="text-md text-black-500">
          ...
          <AlertDialog open={recommededAlert} onOpenChange={setRecommededAlert}>
            <AlertDialogContent>
              <AlertDialogHeader className="flex flex-col items-center justify-center">
                ...
                <AlertDialogDescription className="text-center">
                  DL recommendation successfully.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="flex justify-center mt-4">
                <AlertDialogAction
                  onClick={recommededHandle}
